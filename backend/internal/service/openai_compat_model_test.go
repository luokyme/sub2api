...
func TestForwardAsAnthropic_StripsAnthropicBillingHeaderFromInstructions(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.4","max_tokens":16,"system":[{"type":"text","text":"x-anthropic-billing-header: cc_version=2.1.114.45a; cc_entrypoint=cli; cch=abcde;"},{"type":"text","text":"client-system"}],"messages":[{"role":"user","content":"hello"}],"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_strip_billing_header"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "client-system", gjson.GetBytes(upstream.lastBody, "instructions").String())
}
